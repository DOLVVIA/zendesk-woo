...
function renderStripeCharges(charges, container, panel) {
  ...
  charges.forEach(c => {
    ...
    const success = c.status === 'succeeded';
    const li = document.createElement('li');
    li.innerHTML = `
      <div class="payment-info">
        <span>${c.metadata?.products||c.description||c.id} — ${amount} €</span>
        <span class="badge ${success?'success':'failed'}">${success?'Exitoso':'Fallido'}</span>
      </div>
    `;
    const btnFull = document.createElement('button');
    btnFull.innerText = 'Reembolso completo';
    btnFull.disabled = !success;
...
async function refundPayPal(captureId, amount, currency, panel) {
  try {
    ...
    if (json.success) {
      ...
      await loadPedidos();  // recarga la vista
    } else {
...
// 3) Renderiza PayPal igual que Stripe
function renderPayPalTransactions(txs, container, panel) {
  ...
  txs.forEach(tx => {
    ...
    // --- Botón + formulario reembolso parcial ---
...
async function loadPedidos() {
  ...
  try {
    const res = await fetch(`${API_BASE}/buscar-pedidos?email=${encodeURIComponent(email)}`);
    ...
    pedidos.forEach(async pedido => {
      ...
      panel.innerHTML = `
        ...
        <p><strong>Dirección:</strong> ${b.address_1||''}, ${b.city||''}, ${b.postcode||''}</p>
        <hr>
      `;
      ...
      // Controles extra
      panel.innerHTML += `
        <button class="btn-add-item" data-order-id="${pedido.id}">Añadir artículo</button>
        <button class="btn-change-status" data-order-id="${pedido.id}">Cambiar estado</button>
        <button class="btn-edit-address" data-order-id="${pedido.id}">Editar Dirección</button>
        <form class="form-address" data-order-id="${pedido.id}" style="display:none;"></form>
        <hr>
      `;
      ...
      const charges = await loadStripeCharges(pedido.email);
...
if (captureId) {
  ...
  paypalSection.innerHTML += '<p>No hay transacción PayPal para este pedido.</p>';
    ...
    resultados.innerHTML = `<p style="color:red;">Error al buscar pedidos.</p>`;
    ajustarAlto();
  }
}
...
document.addEventListener('click', async e => {
  ...
  form.querySelector('.cancel-edit').onclick = () => {
    form.remove(); prodDiv.style.display = ''; ajustarAlto();
  };
  ...
  form.onsubmit = async ev => {
    ...
    try {
      ...
      form.remove(); prodDiv.style.display = ''; await loadPedidos();
    } catch {
...
document.addEventListener('click', async e => {
  ...
  const btn = e.target; const panel = btn.closest('.panel');
  ...
  if (!panel.querySelector('.confirm-delete')) {
    ...
    div.querySelector('.cancel-delete').onclick = () => { div.remove(); ajustarAlto(); };
    div.querySelector('.confirm-delete').onclick = async () => {
      try {
        ...
        showMessage(panel, 'Artículo eliminado.'); await loadPedidos();
      } catch { showMessage(panel, 'Error al eliminar.', 'error'); }
    };
    ajustarAlto();
  }
});
...
document.addEventListener('click', async e => {
  ...
  const btn = e.target; const panel = btn.closest('.panel'); if (panel.querySelector('.add-form')) return;
  await loadProducts();
  ...
  form.querySelector('.cancel-add').onclick = () => { form.remove(); ajustarAlto(); };
  ...
  prodSel.onchange = async () => {
    if (!prodSel.value) { varSel.innerHTML = `<option>– N/A –</option>`; varSel.disabled = true; ajustarAlto(); return; }
    try {
      ...
      ajustarAlto();
    } catch { console.error('Error cargando variaciones'); }
  };
  ...
  form.onsubmit = async ev => {
    ...
    const custom = fd.get('custom_total'); if (custom) payload.total = parseFloat(custom).toFixed(2);
    try {
      await fetch(`${API_BASE}/anadir-item?order_id=${btn.dataset.orderId}`, {
        method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)
      });
      showMessage(panel,'Artículo añadido.'); form.remove(); await loadPedidos();
    } catch { showMessage(panel,'Error al añadir artículo.','error'); }
  };
});
...
document.addEventListener('click', e => {
  if (e.target.matches('.btn-change-status')) {
    ...
    const sel = document.createElement('select'); sel.className = 'select-status'; sel.dataset.orderId = orderId;
    orderStatuses.forEach(s => {
      sel.innerHTML += `<option value="${s.slug}" ${s.slug===e.target.dataset.status?'selected':''}>${s.name}</option>`;
    });
    const btnA = document.createElement('button'); btnA.className='btn-status-accept'; btnA.innerText='Aceptar';
    const btnC = document.createElement('button'); btnC.className='btn-status-cancel'; btnC.innerText='Cancelar';
    panel.append(sel, btnA, btnC); ajustarAlto();
  }
...
document.addEventListener('click', async e => {
  ...
  } catch { showMessage(panel,'Error cambiando estado.','error'); }
  sel.remove(); e.target.remove(); panel.querySelector('.btn-status-cancel')?.remove(); await loadPedidos();
});
...
document.addEventListener('click', async e => {
  ...
  try {
    await fetch(`${API_BASE}/editar-direccion?order_id=${form.dataset.orderId}`, {
      method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)
    });
    showMessage(panel,'Dirección actualizada.'); await loadPedidos();
  } catch { showMessage(panel,'Error guardando dirección.','error'); }
});